...
class FullyConnected:
    ...
    def forward(self, ch, val_mode=False):  # ch: (2 * num_ant, 1), organized as (h_r, h_i)
        if val_mode:
            w_r = (1 / np.sqrt(self.num_ant)) * np.cos(self.thetas) # (num_beam, num_ant)
            w_i = (1 / np.sqrt(self.num_ant)) * np.sin(self.thetas)
            W_block = np.block([[w_r, w_i], [-w_i, w_r]])   # [w_r w_i ; -w_i w_r]
    ...
    def update(self, lr=0.1, beta_1=0.9, beta_2=0.999):
        ...
        self.thetas = self.thetas - lr * self.grad #np.power(v_t_corr, -0.5) * m_t_corr
        return self.thetas
